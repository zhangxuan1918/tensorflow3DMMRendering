...
void _TNorm2VNorm(double* normv, int nver, double* normt, int* tri, int ntri)
{
	...
	for(i = 0; i < ntri; i++)
	{
		int trianglePt0Index = tri[3*i];
		int trianglePt1Index = tri[3*i+1];
		int trianglePt2Index = tri[3*i+2];

		for(j = 0; j < 3; j++)
		{
			normv[3*trianglePt0Index+j] += normt[3*i+j];
			normv[3*trianglePt1Index+j] += normt[3*i+j];
			normv[3*trianglePt2Index+j] += normt[3*i+j];
		}
