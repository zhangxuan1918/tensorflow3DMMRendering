...
cdef extern from "mesh_core.h":
    ...
    void _TNorm2VNorm(double* normv, int nver, double* normt, int* tri, int ntri)
...
def tnorm_to_vnorm(
    np.ndarray[double, ndim=2, mode = "c"] normv not None,
    int nver,
    np.ndarray[double, ndim=2, mode = "c"] normt not None,
    np.ndarray[int, ndim=2, mode = "c"] tri not None,
    int ntri
    ):

    _TNorm2VNorm(<double*> np.PyArray_DATA(normv), nver, <double*> np.PyArray_DATA(normt),
        <int*> np.PyArray_DATA(tri), ntri)