...
def generate_vertex_norm(vertices, triangles, nver, ntri):
    '''
    generate norm for each vertex using norms of triangles
    ...
    # norm of vertices
    N = np.zeros((nver, 3))
    N = tnorm_to_vnorm(normv=N, nver=nver, normt=norm_tri, triangles=triangles, ntri=ntri)
    # mag of shape (nver, 1)
...
def tnorm_to_vnorm(normv, nver, normt, triangles, ntri):
    for i in range(ntri):
        normv[triangles[i, 0], :] += normt[i, :]
        normv[triangles[i, 1], :] += normt[i, :]
        normv[triangles[i, 2], :] += normt[i, :]
