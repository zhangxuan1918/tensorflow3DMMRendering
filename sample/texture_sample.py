...
def main():
    ...
    texture_tmp = imageio.imread(os.path.dirname(__file__) + '/cat.jpg') / 255.
    texture = tf.constant(texture_tmp, dtype=tf.float32)
    ...
    with session.as_default():
        ...
        imageio.imsave('textured_simple.jpg', (pixels_eval * 255).astype(np.uint8))

        # imageio.imsave('textured_simple.jpg', (texture.eval() * 255).astype(np.uint8))
